# -*- coding: utf-8 -*-
"""PCD_parte_3.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1-0yGnthtI12kDlKm2uo3hRIJ1ZryHTpF

# __Funciones__

Las funciones son bloques de código que se ejecutan únicamente cuando son llamados. Nos permiten construir programas más modulares y reutilizar código.

## __Aspectos básicos__

### __Declarar una función__

Para definir una función en Python debemos utilizar la palabra reservada `def` y utilizar el nivel de indentación adecuado
"""

def mi_funcion():# def para definir la funcion
  print('Mi primera función')

"""### __Llamar una función__
Para llamar a una función debemos utilizar su identificador:
"""

mi_funcion()

"""Podemos llamarla cuantas veces lo necesitemos"""

range(5)

for elemento in range(5):
  print(elemento)

for i in range(10):
  mi_funcion()

"""Cuando se llama a una función y no se utilizan los parentesis, lo que se hace es pasar una referencia a la función (como si fuera una variable cualquiera). Las funciones en Python son objetos de tipo `callable` que significa que pueden ser ejecutados."""

mi_funcion

a = mi_funcion
a()

"""### __Retorno de valores__

Para comunicarse con otras partes del código las funciones pueden devolver valores al proceso principal a través del la palabra reservada `return`
"""

def prueba():
  print("Aprendamos Python")

  return 'Esto es una valor retornado por la función'

val = prueba()
#print(val)

print(val)

val = prueba()

"""Los valores retornados se tratan como un valor del tipo de dato retornado"""

def prueba():
  print("Segunda prueba")
  return 'Esto es una valor retornado por la función'

val = prueba() + ' y un String adicional'
#print(val)

val

def prueba():

  return [1,2,3,4,5]

#print(prueba())
#print(prueba()[-1])
print(prueba()[1:4])

"""Python pemite retornar más de un valor mediante el uso de tuplas"""

def operaciones():
  a = 2.0
  b = 5.0
  #a = int(float(input("ingrese valor 1: ")))
  #b = int(float(input("ingrese valor 2: ")))

  return a + b, a - b, a * b, a/b

print(operaciones())
print(type(operaciones()))
print(operaciones()[0])
print(operaciones()[1])
print(operaciones()[0:4:2])

"""Podemos realizar el desempaquetamiento de los valores que retorna la función si lo consideramos conveniente"""

def operaciones():
  a = 2.0
  b = 5.0
  return a + b, a - b, a * b

(suma, resta, producto) = operaciones()

print(operaciones())
print(suma, resta, producto)

"""Vale la pena mencionar que una función sin la instrucción `return` equivale a tener un `return None`"""

def mi_funcion():
  a = 2

def mi_funcion2():
  a = 2
  return None

val = mi_funcion()
val2 = mi_funcion()

print(val, val2)

"""Si por alguna razón se requiere declarar un función pero aún no se conocen las instrucciones que ejecutará es posible utilizar la palabra reservada `pass` para evitar generar un error"""

def prueba():
  pass

prueba()

"""### __Docstrings__

Las `docstrings` son cadena de caracteres que aparecen justo después de la definición de una función, método, clase o módulo. Sirven como parte de la documentación de nuestros programas
"""

def mi_funcion():
  '''Este es un ejemplo de docstring para Python'''
  print('Mi primera función')

mi_funcion()

def saludar():
  '''La funcion lo que hace es saludar al usuario
  input:  El nombre del usuario, sexo, edad
  output: La salida esperada de la fucion.
  '''
  print('Mi primera función')

"""Podemos acceder a los `docstrings` de una función a través del método mágico `__doc__`"""

saludar()

saludar.__doc__

"""También podemos acceder"""

help(saludar)

"""## __Argumentos__

Los argumentos (o parámetros) hacen referencia al conjunto de variables que se pasa a una función para que esta pueda ejecutarse
"""

def saludar(nombre, apellido):
  print(f'¡Hola, {nombre} {apellido}!')

saludar('Andres',"Patiño")

"""Al llamar una función se deben especificar el número correcto de argumentos, de lo contrario se generará un error."""

def saludar(nombre, apellido):
  print(f'¡Hola, {nombre} {apellido}!')

saludar('Pedro')

"""### __Argumentos por valor y por referencia__

Según el tipo de dato que pasemos a una función podemos diferenciar dos comportamientos:

* __Paso por valor__: crea una copia de la variable dentro de la función.
* __Paso por referencia__: se maneja directamente la variable; los cambios realizados dentro de la función se conservarán por fuera de la función

En general, los tipos simples se pasan por valor: enteros, flotantes, cadenasy booleanos.
"""

def duplicar(numero):
    numero += numero
    print('Al interior de la función:', numero)

numero = 30
duplicar(numero)
print('Fuera de la función:', numero)

"""Para modificar los tipos simples podemos reasignarlos:"""

print(numero)

def duplicar(numero):
    numero += numero
    return numero
print(numero)
numero = duplicar(numero)
print(numero)

"""Si embargo si declaramos una variable dentro de la función con el mismo nombre la conexión con la lista inicial se perderá"""

lista= [1,8,9,7,6]

def cambiar_lista(lista):
   '''Esta función combina dos listas'''
   lista = [1,2,3,4]
   print("Valores dentro de la función:", lista)
   return


cambiar_lista(lista);
print('Valores al fuera de la función', lista)

x = [2,3,4,5]

x+[8]

"""### __Alcance local y alcance global__

Dependiendo del lugar donde sea declarada una variable puede ser global o local. Las variable globales son aquellas declaradas por fuera de cualquier función y pueden ser utilizadas en cualquier parte del código
"""

def imprimir():
  print(s)

# Variable global
s = "Esta cadena sirve para el ejemplo"
imprimir()

"""Si definimos una variable con el mismo nombre al interior de una función esta imprimirá el valor definido en su interior y no el de la variable global"""

def imprimir():
    s = 'Otra cadena' # alcance local
    print(s)

s = 'Esta cadena sirve para el ejemplo' # alcance global
imprimir()
print(s)

def imprimir():
    #print(s)

    # El programa nos mostrará un ERROR
    s = "Otra cadena"

    print(s)

s = 'Esta cadena sirve para el ejemplo'
imprimir()
print(s)

"""Podemos usar la palabra reservada `global` si queremos hacer referencia a una variable globlal al interior de una función"""

def imprimir():
    global s  #global, palabra del sistema, le da alcance global a la variaable definida
    print(s)
    s = "Otra cadena"
    print(s)

s = 'Esta cadena sirve para el ejemplo'
print(s)
print("++++++++")
imprimir()
print("+++++++")
print(s)

"""También la podemos utilizar para crear variables globales al interior de una función."""

def imprimir():
    global s
    s = 'Esta cadena sirve para el ejemplo'
    print(s)

imprimir()
print(s)



"""### __Argumentos por defecto__

Podemos generar funciones con argumentos por defecto asignando a los argumentos un identificador
"""

def perimetro_rectangulo(altura=1, base=1):
  perimetro = 2 * base  + 2 * altura
  return perimetro

#print(perimetro_rectangulo())
print(perimetro_rectangulo(2,3))
#print(perimetro_rectangulo(base=3))
#print(perimetro_rectangulo(altura=3))
#print(perimetro_rectangulo(altura=5, base=5))
#print(perimetro_rectangulo(base=5,altura=5))

"""Podemos combinar argumentos por valor y aquellos que usan un identificador. La única condición es que los argumentos con valor siempre deben ir al principio"""

def saludo(nombre, pais = 'Colombia'):
  print(f'Hola, soy {nombre} de {pais}')

#saludo('juan')
#saludo('María', 'Argentina')
#saludo('Luis', pais='México')

saludo(pais='México')

"""Debemos recordar que los argumentos por valor son obligatorios. Si hacemos un llamado a una función sin el argumento con valor se generará un error"""

def saludo(nombre, pais = 'Colombia'):
  print(f'Hola, soy {nombre} de {pais} ')


saludo(pais='Argentina')

"""### __Argumentos de longitud variable: `*args`__
Puede ocurrir una situación en la cual no conozcamos con anticipación cuantos argumentos va a recibir una función. En este caso podemos crear una tupla dinámica definiendo los parámetros con un asterisco `*`

"""

def argumentos_variables(*args):
  for i in args:
    print(i, end=' ')


argumentos_variables(5, 'hola', 'True', [1,2,3], {'Edad':23, 'Rh': 'A+'})
#print()
#argumentos_variables(5, 'hola', 'True', [1,2,3])
#print()
#argumentos_variables(5, 'hola', 'True')
#print()
#argumentos_variables()

print(argumentos_variables())

sum(list(range(10)))/len(range(10))

def funcion_promedio (*args):
  sum=0
  for i in args:
    sum+=i
    print(sum)
  print(sum/len(args))

funcion_promedio(1,2,3,4,5,6,7,8,9)

"""### __Argumentos de longitud variable: `**kwargs`__
También es posible recibir una cantidad indeterminada de argumentos con indetificador definiendo los parámetros con 2 asterísticos `**`. En este caso la función recibirá un diccionario de la forma `<identificador:valor>`
"""

argumentos_variables(numero=6,booleano=True,lista=[1,2,3] )

import pandas as pd
diccionario = {"Nombre":["Pepito 1", "Pepito 2", "Pepito 3"], "Documento": [111,222,333]}
df = pd.DataFrame(diccionario)

df

def argumentos_variables(**kwargs):
  for key, value in kwargs.items():
    print((key, value), end=' ')

#argumentos_variables(numero=5, cadena='hola', booleano=True, lista=[1,2,3], dic={'Edad':23, 'Rh': 'A+'})
#print()
#argumentos_variables(numero=5, cadena='hola', booleano=True, lista=[1,2,3])
#print()
#argumentos_variables(numero=5, cadena='hola', booleano=True)

"""Si se requiere una función que acepte ambos tipos de argumentos indeterminados podemos hacerlo. La única condición es llamar primero a los argumentos por valor y luego a aquellos con indentificador"""

def super_funcion(variable, variaable2=2,*args, **kwargs):
  '''Una función que puede recibir cualquier combinación de argumentos '''
  print(variable)
  print(variaable2)
  total = 0
  for arg in args:
    total += arg
    print(args)
  print("Total args = ", total, end=' ')

  for key, value in kwargs.items():
    print((key, value), end=' ')



super_funcion("a","b",10, 50, -1, 1.56, 10, 20, 300, nombre="Katherine", edad=27)

"""## __Funciones anidadas__
Python, al igual que la mayoría de lenguajes de programación permite crear funciones anidadas (una función dentro de otra función). Es posible utilizar las funciones anidadas para esconder la función interna del mundo exterior e implementar la encapsulación de datos.

"""

def funcion_externa():
   valor =2

   def funcion_interna(valor, cadena):
     print('funcion interna:', valor, cadena)

   funcion_interna(valor, 'prueba1')
   funcion_interna(valor, 'prueba2')
   funcion_interna(valor, 'prueba3')

funcion_externa()

funcion_externa()

"""Las funciones internas pueden acceder a las variables de la función externa (variables no locales)."""

def funcion_externa():
  valor = 1
  def funcion_interna(valor, cadena):
    valor += 1
    print('funcion interna:', valor, cadena)

  funcion_interna(valor, 'prueba1')
  funcion_interna(valor, 'prueba2')
  print('fucnion externa:', valor)

funcion_externa()

"""Si necesitamos modicar las variables de la función externa debemos utilizar la palabra reservada `nonlocal`"""

def funcion_externa():
  valor =1
  def funcion_interna(cadena):
    nonlocal valor # nonlocal, palabra reservada del sistema, hace referencia a las variables no globales
    valor += 1
    print(valor, cadena)

  funcion_interna('prueba1')
  funcion_interna('prueba2')
  print('fucnion externa:', valor)

funcion_externa()

"""## __Funciones recursivas__

Las funciones recursivas son aquellas que se llaman a si mismas durante su proceso de ejecución. Un ejemplo típico es el cálculo del factorial
"""

def factorial(n):
  if n==1 :
    return 1
  return n * factorial(n - 1)

def otro_factorial(n):
  return i if n == 1 else n * factorial(n - 1)

print(factorial(4))
print(otro_factorial(4))

def factorial(n):
  if n==1 :
    return 1
  return n * factorial(n - 1)

factorial(5)

factorial(3)

"""5! = 5 * 4 * 3 * 2 * 1

La sumatoria de los valores de `1` hasta `n`
"""

def sumatoria(n):
  return 1 if n == 1 else n + sumatoria(n - 1)

print(sumatoria(4))
print(sumatoria(5))

"""Una cuenta regresiva:"""

def cuenta_regresiva(n):
  if n >= 0:
    print(n)
    cuenta_regresiva(n - 1)
  else:
    print('¡Boooooom! la bomba estalló')

cuenta_regresiva(10)

"""## __Funciones incorpordas__

Python cuenta con las siguientes funciones incorporadas:

![imagen.png](data:image/png;base64,)

En este enlace se encuentra la [documentación oficial](https://)
"""

lista = [1,2,3,4,5,61,1000,500,0,-6]

import numpy as np

y = (1,2,3)

"""Escribir una función que reciba  cinco parametros como valor y retorne el valor máximo, el valor minimo, el promedio y la suma

##__Importando funciones__

Una tarea común en cualquier proceso de desarrollo es utilizar librerías y paquetes que continene funcionalidades ya implementadas y que nos pueden ahorrar algo de tiempo. Para esto podemos utilizar las palabras reservadas `import` y `from`. Veamos un ejemplo utilizando el módulo [Math](https://docs.python.org/3/library/math.html) de Python.
"""

import math

print(math.factorial(5))

"""También podemos importar la función directamente combinando las palabras reservadas `from` e `import`"""

from math import factorial

print(factorial(5))

"""Es posible crear un alias para las importaciones que realicemos con la palabra reservada `as`

"""

import math as mathematical_functions

print(mathematical_functions.factorial(5))

"""### Ejemplo de funcion para calcular el indice de masa corporal"""

def calcular_imc():
    """
    Calcula el Índice de Masa Corporal (IMC) de un usuario.
    Además, permite calcular el peso a partir del IMC y la altura,
    o la altura a partir del IMC y el peso, según los datos proporcionados.

    El IMC se calcula con la fórmula: IMC = Peso / Altura^2
    """

    # Solicitar al usuario la opción deseada
    print("Seleccione la opción:")
    print("1. Calcular IMC")
    print("2. Estimar peso a partir del IMC y la altura")
    print("3. Estimar altura a partir del IMC y el peso")

    opcion = int(input("Ingrese el número de la opción seleccionada: "))

    # Opción 1: Calcular IMC
    if opcion == 1:
        # Solicitar el peso y la altura al usuario
        peso = float(input("Ingrese su peso en kilogramos: "))
        altura = float(input("Ingrese su altura en metros: "))

        # Calcular el IMC usando la fórmula: IMC = peso / altura^2
        imc = peso / (altura ** 2)

        # Imprimir el resultado del IMC
        print("Su IMC es:", imc)

    # Opción 2: Estimar peso a partir del IMC y la altura
    elif opcion == 2:
        # Solicitar el IMC y la altura al usuario
        imc = float(input("Ingrese su IMC: "))
        altura = float(input("Ingrese su altura en metros: "))

        # Calcular el peso usando la fórmula: Peso = IMC * altura^2
        peso_estimado = imc * (altura ** 2)

        # Imprimir el peso estimado
        print("Su peso estimado es:", peso_estimado, "kilogramos")

    # Opción 3: Estimar altura a partir del IMC y el peso
    elif opcion == 3:
        # Solicitar el IMC y el peso al usuario
        imc = float(input("Ingrese su IMC: "))
        peso = float(input("Ingrese su peso en kilogramos: "))

        # Calcular la altura usando la fórmula: Altura = sqrt(Peso / IMC)
        altura_estimada = (peso / imc) ** 0.5

        # Imprimir la altura estimada
        print("Su altura estimada es:", altura_estimada, "metros")

    # Si el usuario ingresa una opción no válida
    else:
        print("Opción no válida. Por favor, seleccione 1, 2 o 3.")

# Ejemplo de uso de la función
calcular_imc()

"""### Como ejercicio se debe implementar en funciones los ejercicios dejados la sesión anterior"""

def imc(edad, altura, masa):
    """
    Calcula el Índice de Masa Corporal (IMC) de una persona en función de su altura y peso,
    e informa sobre su clasificación en distintas categorías de IMC según la Organización Mundial de la Salud (OMS).

    Además, informa si la persona es menor o mayor de edad basándose en la edad proporcionada.

    Parámetros:
    - edad (int): La edad de la persona en años.
    - altura (float): La altura de la persona en metros.
    - masa (float): El peso de la persona en kilogramos.

    La función imprime si la persona es menor o mayor de edad, su IMC calculado,
    y su categoría correspondiente según su IMC.
    """

    # La estatura se mantiene como se ingresó.
    est = altura

    # La masa en kilogramos se mantiene en formato flotante para permitir decimales.
    # Calculo del IMC: masa (en kilogramos) dividida por la estatura (en metros) elevada al cuadrado.
    IMC = masa / est**2

    # Determinación de si la persona es menor o mayor de edad.
    # Si la edad es menor de 18, se considera menor de edad; de lo contrario, mayor de edad.
    if edad < 18:
        print("Usted es menor de edad")
    else:
        print("Usted es mayor de edad")

    # Imprimimos el valor calculado del IMC.
    print("IMC: " + str(IMC))

    # Clasificación del IMC según la OMS:
    # Cada rango de IMC se clasifica en categorías como delgadez, normal, sobrepeso y obesidad.
    if IMC >= 0 and IMC <= 15.99:
        print("Delgadez severa")
    elif IMC >= 16.00 and IMC <= 16.99:
        print("Delgadez moderada")
    elif IMC >= 17.00 and IMC <= 18.49:
        print("Delgadez leve")
    elif IMC >= 18.50 and IMC <= 24.99:
        print("Normal")
    elif IMC >= 25.00 and IMC <= 29.99:
        print("Sobrepeso")
    elif IMC >= 30.00 and IMC <= 34.99:
        print("Obesidad leve")
    elif IMC >= 35.00 and IMC <= 39.99:
        print("Obesidad media")
    elif IMC >= 40.00:
        print("Obesidad mórbida")

# Solicitar al usuario los datos necesarios: edad, altura y masa.
edad = int(input("Su edad en años por favor: "))
altura = float(input("Su altura en metros por favor: "))
masa = float(input("Su masa en kilogramos por favor: "))

# Llamar a la función imc() con los valores ingresados.
imc(edad, altura, masa)

def comparar_impacto_combinaciones():
    """
    Compara el impacto de dos combinaciones de centros de atención de diferentes tipos
    en términos del número de personas que pueden atender.

    La función solicita al usuario que ingrese dos combinaciones (cantidad y tipo de centro),
    calcula el número de personas impactadas por cada combinación y luego compara ambos impactos.

    Tipos de centros y su capacidad de atención:
    - Centro de tipo A: 6530 personas
    - Centro de tipo B: 8324 personas
    - Centro de tipo C: 11450 personas

    No recibe parámetros y no retorna ningún valor. Imprime el resultado de la comparación.
    """

    # Definir la capacidad de atención de cada tipo de centro
    capacidad_A = 6530
    capacidad_B = 8324
    capacidad_C = 11450

    # Pedir al usuario las combinaciones para la primera entrada
    print("Ingrese la primera combinación (ejemplo: 1 centro de tipo A):")
    centros_1 = input("Combinación 1: ").split()
    cantidad_1 = int(centros_1[0])  # Convertimos la cantidad a entero
    tipo_1 = centros_1[-1].upper()  # Convertimos el tipo de centro a mayúsculas para uniformidad

    # Pedir al usuario las combinaciones para la segunda entrada
    print("Ingrese la segunda combinación (ejemplo: 2 centros de tipo B):")
    centros_2 = input("Combinación 2: ").split()
    cantidad_2 = int(centros_2[0])  # Convertimos la cantidad a entero
    tipo_2 = centros_2[-1].upper()  # Convertimos el tipo de centro a mayúsculas para uniformidad

    # Calcular el número de personas impactadas por cada combinación
    if tipo_1 == 'A':
        impacto_1 = cantidad_1 * capacidad_A
    elif tipo_1 == 'B':
        impacto_1 = cantidad_1 * capacidad_B
    elif tipo_1 == 'C':
        impacto_1 = cantidad_1 * capacidad_C
    else:
        impacto_1 = 0  # Si el tipo no es válido, consideramos impacto 0

    if tipo_2 == 'A':
        impacto_2 = cantidad_2 * capacidad_A
    elif tipo_2 == 'B':
        impacto_2 = cantidad_2 * capacidad_B
    elif tipo_2 == 'C':
        impacto_2 = cantidad_2 * capacidad_C
    else:
        impacto_2 = 0  # Si el tipo no es válido, consideramos impacto 0

    # Comparar los impactos y mostrar el resultado
    if impacto_1 > impacto_2:
        print("La combinación 1 impacta más usuarios.")
    elif impacto_2 > impacto_1:
        print("La combinación 2 impacta más usuarios.")
    else:
        print("Ambas combinaciones impactan la misma cantidad de usuarios.")

# Ejemplo de uso de la función
comparar_impacto_combinaciones()

def calcular_valores_productos():
    """
    Calcula el costo total y los promedios de tres productos antes y después
    de aplicar los costos de envío específicos por país.

    Se solicitan tres valores de productos y se aplican las siguientes tarifas de envío:
    - Producto 1: 15% de costo adicional por envío
    - Producto 2: 18% de costo adicional por envío
    - Producto 3: 12% de costo adicional por envío

    La función imprime el valor total de los productos con los costos de envío
    incluidos y los promedios de los productos antes y después del envío.

    No recibe parámetros ni retorna valores. Los resultados se imprimen en consola.
    """

    # Solicitar el valor original de cada producto
    producto1 = float(input("Introduce el valor del primer producto: "))
    producto2 = float(input("Introduce el valor del segundo producto: "))
    producto3 = float(input("Introduce el valor del tercer producto: "))

    # Calcular el valor adicional debido al costo de envío para cada producto
    envio_producto1 = producto1 * 0.15  # Costo de envío del 15% para el producto 1
    envio_producto2 = producto2 * 0.18  # Costo de envío del 18% para el producto 2
    envio_producto3 = producto3 * 0.12  # Costo de envío del 12% para el producto 3

    # Sumar el costo de envío al valor original de cada producto
    total_producto1 = producto1 + envio_producto1
    total_producto2 = producto2 + envio_producto2
    total_producto3 = producto3 + envio_producto3

    # Calcular el valor total de los tres productos con el costo de envío incluido
    valor_total = total_producto1 + total_producto2 + total_producto3

    # Calcular el promedio de los valores originales de los tres productos antes del envío
    promedio_antes = (producto1 + producto2 + producto3) / 3

    # Calcular el promedio de los valores de los productos después de incluir los costos de envío
    promedio_despues = valor_total / 3

    # Imprimir los resultados en la consola
    print("El valor total de los tres productos con costos de envío es:", valor_total)
    print("El promedio de los productos antes del envío es:", promedio_antes)
    print("El promedio de los productos después del envío es:", promedio_despues)

# Ejemplo de uso de la función
calcular_valores_productos()

def evaluar_signos_vitales():
    """
    Evalúa los signos vitales de un paciente, comparándolos con los rangos normales
    para determinar si se encuentran en un estado saludable o fuera de rango.

    Se solicitan los siguientes valores:
    - IMC (Índice de Masa Corporal)
    - Frecuencia cardíaca (latidos por minuto)
    - Presión arterial (mm Hg)
    - Saturación de oxígeno (%)

    La función imprimirá el estado de cada valor (si está en rango normal o fuera de rango).

    No recibe parámetros ni retorna valores. Los resultados se imprimen en la consola.
    """

    # Solicitar los datos del paciente
    imc = float(input("Introduce el IMC del paciente: "))
    frecuencia_cardiaca = float(input("Introduce la frecuencia cardíaca del paciente (latidos por minuto): "))
    presion_arterial = float(input("Introduce la presión arterial del paciente (mm Hg): "))
    saturacion_oxigeno = float(input("Introduce la saturación de oxígeno del paciente (%): "))

    # Comparar los valores con los rangos normales

    # Verificación del IMC (debe estar entre 22.1 y 24.9)
    if 22.1 <= imc <= 24.9:
        imc_normal = "IMC está en rango normal"
    else:
        imc_normal = "IMC está fuera de rango"

    # Verificación de la frecuencia cardíaca (debe ser menor a 70)
    if frecuencia_cardiaca < 70:
        frecuencia_cardiaca_normal = "Frecuencia cardíaca es muy buena"
    else:
        frecuencia_cardiaca_normal = "Frecuencia cardíaca fuera de rango"

    # Verificación de la presión arterial (debe estar entre 80 y 120)
    if 80 <= presion_arterial <= 120:
        presion_arterial_normal = "Presión arterial está en rango normal"
    else:
        presion_arterial_normal = "Presión arterial está fuera de rango"

    # Verificación de la saturación de oxígeno (debe ser mayor a 95)
    if saturacion_oxigeno > 95:
        saturacion_oxigeno_normal = "Saturación de oxígeno está en rango normal"
    else:
        saturacion_oxigeno_normal = "Saturación de oxígeno está fuera de rango"

    # Imprimir los resultados de las evaluaciones
    print(imc_normal)
    print(frecuencia_cardiaca_normal)
    print(presion_arterial_normal)
    print(saturacion_oxigeno_normal)

# Ejemplo de uso de la función
evaluar_signos_vitales()

def calcular_pacientes_priorizados():
    """
    Calcula la cantidad de pacientes que no pueden ser asignados equitativamente
    entre tres médicos y, por lo tanto, estarán en una lista de priorizados.

    La función solicita al usuario que ingrese el número de pacientes atendidos
    por tres médicos y calcula cuántos pacientes quedan sin asignar de manera
    equitativa al dividir el total entre los tres médicos.

    No recibe parámetros ni retorna valores. Los resultados se imprimen en la consola.
    """

    # Solicitar el número de pacientes atendidos por cada médico
    pacientes_medico1 = int(input("Introduce el número de pacientes atendidos por el primer médico: "))
    pacientes_medico2 = int(input("Introduce el número de pacientes atendidos por el segundo médico: "))
    pacientes_medico3 = int(input("Introduce el número de pacientes atendidos por el tercer médico: "))

    # Calcular el total de pacientes atendidos sumando los pacientes de los tres médicos
    total_pacientes = pacientes_medico1 + pacientes_medico2 + pacientes_medico3

    # Determinar cuántos pacientes se pueden repartir equitativamente entre los tres médicos
    pacientes_por_medico = total_pacientes // 3

    # Calcular la cantidad de pacientes que quedan sin asignar de manera equitativa (lista de priorizados)
    pacientes_priorizados = total_pacientes % 3

    # Imprimir el número de pacientes que estarán en la lista de priorizados
    print("La cantidad de pacientes en la lista de priorizados es:", pacientes_priorizados)

# Ejemplo de uso de la función
calcular_pacientes_priorizados()

def determinar_cuadrante():
    """
    Determina en qué cuadrante del gráfico político se encuentra una persona en función de su tendencia política (eje horizontal)
    y su visión económica (eje vertical). También maneja los casos especiales cuando la persona está en alguno de los ejes o en el origen.

    La función solicita al usuario que ingrese dos puntuaciones:
    - Tendencia política (valor entre -50 y 50)
    - Visión económica (valor entre -50 y 50)

    La función imprime el cuadrante en el que se encuentra la persona o si está en un eje o en el origen.
    No recibe parámetros ni retorna valores.
    """

    # Solicitar al usuario que introduzca la puntuación de la tendencia política (eje horizontal) y la visión económica (eje vertical)
    politica = int(input("Introduce la puntuación de la tendencia política (-50 a 50): "))
    economica = int(input("Introduce la puntuación de la visión económica (-50 a 50): "))

    # Determinar en qué cuadrante se encuentra la persona utilizando condicionales
    if politica < 0 and economica > 0:
        cuadrante = "Izquierda Autoritaria"
    elif politica > 0 and economica > 0:
        cuadrante = "Derecha Autoritaria"
    elif politica < 0 and economica < 0:
        cuadrante = "Izquierda Libertaria"
    elif politica > 0 and economica < 0:
        cuadrante = "Derecha Libertaria"
    elif politica != 0 and economica == 0:
        cuadrante = "En el eje horizontal (económico), sin inclinación autoritaria/libertaria"
    elif politica == 0 and economica != 0:
        cuadrante = "En el eje vertical (autoritarismo/libertarismo), sin inclinación económica"
    else:
        cuadrante = "En el origen (sin inclinación política ni económica)"

    # Imprimir el cuadrante o eje en el que se encuentra la persona
    print("La persona se encuentra en:", cuadrante)

# Ejemplo de uso de la función
determinar_cuadrante()

def calcular_riesgo_corazon():
    """
    Calcula el Índice de Masa Corporal (IMC) y determina el nivel de riesgo de problemas del corazón
    basado en la edad y el IMC del paciente.

    La función solicita al usuario que ingrese su altura, peso y edad, calcula su IMC y luego,
    en función de su edad e IMC, determina el nivel de riesgo:
    - Edad < 45 años: IMC < 22 -> Riesgo Bajo, IMC >= 22 -> Riesgo Medio
    - Edad >= 45 años: IMC < 22 -> Riesgo Medio, IMC >= 22 -> Riesgo Alto

    La función imprime el IMC calculado y el nivel de riesgo.
    No recibe parámetros ni retorna valores.
    """

    # Solicitar la entrada de datos al usuario: altura, peso y edad.
    altura = float(input("Introduce tu altura en metros: "))
    peso = float(input("Introduce tu peso en kilogramos: "))
    edad = int(input("Introduce tu edad en años: "))

    # Calcular el IMC (Índice de Masa Corporal) usando la fórmula IMC = peso / (altura^2)
    imc = peso / (altura ** 2)

    # Imprimir el valor calculado del IMC
    print("Tu IMC es:", imc)

    # Determinar el nivel de riesgo basado en la edad y el IMC utilizando condicionales
    if edad < 45:
        # Si la persona tiene menos de 45 años
        if imc < 22:
            riesgo = "Bajo"
        else:
            riesgo = "Medio"
    else:
        # Si la persona tiene 45 años o más
        if imc < 22:
            riesgo = "Medio"
        else:
            riesgo = "Alto"

    # Imprimir el nivel de riesgo calculado
    print("Tu nivel de riesgo de problemas del corazón es:", riesgo)

# Ejemplo de uso de la función
calcular_riesgo_corazon()

def ordenar_atencion_pacientes():
    """
    Ordena a tres pacientes según su grupo poblacional para determinar el orden de atención.
    La prioridad es la siguiente:
    1. Adulto mayor
    2. Niño
    3. Adulto

    La función solicita al usuario que ingrese los grupos poblacionales de tres pacientes
    (Niño, Adulto, Adulto mayor), y los ordena en base a la prioridad mencionada.

    La función imprime el orden de atención final de los pacientes.
    No recibe parámetros ni retorna valores.
    """

    # Solicitar la entrada de los grupos poblacionales de los tres pacientes
    paciente1 = input("Introduce el grupo poblacional del primer paciente (Niño, Adulto, Adulto mayor): ").lower()
    paciente2 = input("Introduce el grupo poblacional del segundo paciente (Niño, Adulto, Adulto mayor): ").lower()
    paciente3 = input("Introduce el grupo poblacional del tercer paciente (Niño, Adulto, Adulto mayor): ").lower()

    # Inicializar variables para almacenar el orden de atención
    primer_paciente = ""
    segundo_paciente = ""
    tercer_paciente = ""

    # Prioridad máxima: Adulto mayor
    if paciente1 == "adulto mayor":
        primer_paciente = "Adulto mayor"
        if paciente2 == "niño" or paciente3 == "niño":
            segundo_paciente = "Niño"
            tercer_paciente = "Adulto"
        else:
            segundo_paciente = "Adulto"
            tercer_paciente = "Adulto"

    elif paciente2 == "adulto mayor":
        primer_paciente = "Adulto mayor"
        if paciente1 == "niño" or paciente3 == "niño":
            segundo_paciente = "Niño"
            tercer_paciente = "Adulto"
        else:
            segundo_paciente = "Adulto"
            tercer_paciente = "Adulto"

    elif paciente3 == "adulto mayor":
        primer_paciente = "Adulto mayor"
        if paciente1 == "niño" or paciente2 == "niño":
            segundo_paciente = "Niño"
            tercer_paciente = "Adulto"
        else:
            segundo_paciente = "Adulto"
            tercer_paciente = "Adulto"

    # Prioridad media: Niño (Si no hay "Adulto mayor")
    elif paciente1 == "niño":
        primer_paciente = "Niño"
        if paciente2 == "niño" or paciente3 == "niño":
            segundo_paciente = "Niño"
            tercer_paciente = "Adulto"
        else:
            segundo_paciente = "Adulto"
            tercer_paciente = "Adulto"

    elif paciente2 == "niño":
        primer_paciente = "Niño"
        if paciente1 == "niño" or paciente3 == "niño":
            segundo_paciente = "Niño"
            tercer_paciente = "Adulto"
        else:
            segundo_paciente = "Adulto"
            tercer_paciente = "Adulto"

    elif paciente3 == "niño":
        primer_paciente = "Niño"
        if paciente1 == "niño" or paciente2 == "niño":
            segundo_paciente = "Niño"
            tercer_paciente = "Adulto"
        else:
            segundo_paciente = "Adulto"
            tercer_paciente = "Adulto"

    # Si no hay "Adulto mayor" ni "Niño", todos son "Adulto"
    else:
        primer_paciente = "Adulto"
        segundo_paciente = "Adulto"
        tercer_paciente = "Adulto"

    # Imprimir el orden de atención de los pacientes
    print("El orden de atención de los pacientes es:", primer_paciente, ",", segundo_paciente, ",", tercer_paciente)

# Ejemplo de uso de la función
ordenar_atencion_pacientes()

def calcular_costos_pacientes():
    """
    Calcula el costo final de los servicios medicos para dos pacientes en funcion del servicio elegido y la categoria a la que pertenecen.
    Aplica descuentos segun la categoria y compara los costos finales entre ambos pacientes.

    Servicios y costos:
    - Mamografia: $26,450
    - Tomografia: $64,740
    - Ecografia: $27,920
    - Resonancia magnetica: $96,370
    - Radiografia convencional: $12,970

    Descuentos por categoria:
    - Categoria A: 40%
    - Categoria B: 30%
    - Categoria C: 10%
    - Categoria D: 5%

    La funcion solicita al usuario que ingrese el servicio y la categoria de dos pacientes.
    Imprime el costo final para cada paciente y compara quien pagara menos.
    """

    # Solicitar el servicio elegido por el primer paciente
    servicio1 = input("Introduce el servicio para el primer paciente (mamografia, tomografia, ecografia, resonancia magnetica, radiografia convencional): ").lower()

    # Solicitar la categoria del primer paciente
    categoria1 = input("Introduce la categoria del primer paciente (A, B, C, D): ").upper()

    # Solicitar el servicio elegido por el segundo paciente
    servicio2 = input("Introduce el servicio para el segundo paciente (mamografia, tomografia, ecografia, resonancia magnetica, radiografia convencional): ").lower()

    # Solicitar la categoria del segundo paciente
    categoria2 = input("Introduce la categoria del segundo paciente (A, B, C, D): ").upper()

    # Inicializar las variables de costo para ambos pacientes
    costo1 = 0
    costo2 = 0

    # Definir el costo del primer servicio
    if servicio1 == "mamografia":
        costo1 = 26450
    elif servicio1 == "tomografia":
        costo1 = 64740
    elif servicio1 == "ecografia":
        costo1 = 27920
    elif servicio1 == "resonancia magnetica":
        costo1 = 96370
    elif servicio1 == "radiografia convencional":
        costo1 = 12970

    # Aplicar el descuento correspondiente segun la categoria del primer paciente
    if categoria1 == "A":
        descuento1 = costo1 * 0.40
    elif categoria1 == "B":
        descuento1 = costo1 * 0.30
    elif categoria1 == "C":
        descuento1 = costo1 * 0.10
    elif categoria1 == "D":
        descuento1 = costo1 * 0.05
    else:
        descuento1 = 0  # Si no se introduce una categoria valida, no se aplica descuento

    # Calcular el costo final del primer paciente
    costo_final1 = costo1 - descuento1

    # Definir el costo del segundo servicio
    if servicio2 == "mamografia":
        costo2 = 26450
    elif servicio2 == "tomografia":
        costo2 = 64740
    elif servicio2 == "ecografia":
        costo2 = 27920
    elif servicio2 == "resonancia magnetica":
        costo2 = 96370
    elif servicio2 == "radiografia convencional":
        costo2 = 12970

    # Aplicar el descuento correspondiente segun la categoria del segundo paciente
    if categoria2 == "A":
        descuento2 = costo2 * 0.40
    elif categoria2 == "B":
        descuento2 = costo2 * 0.30
    elif categoria2 == "C":
        descuento2 = costo2 * 0.10
    elif categoria2 == "D":
        descuento2 = costo2 * 0.05
    else:
        descuento2 = 0  # Si no se introduce una categoria valida, no se aplica descuento

    # Calcular el costo final del segundo paciente
    costo_final2 = costo2 - descuento2

    # Imprimir los resultados
    print("El costo final para el primer paciente es: $", costo_final1)
    print("El costo final para el segundo paciente es: $", costo_final2)

    # Comparar los costos entre los dos pacientes
    if costo_final1 < costo_final2:
        print("El primer paciente pagara menos que el segundo paciente.")
    elif costo_final1 > costo_final2:
        print("El segundo paciente pagara menos que el primer paciente.")
    else:
        print("Ambos pacientes pagaran lo mismo.")

# Ejemplo de uso de la funcion
calcular_costos_pacientes()